...
func TestMuladdrshiftcodemodOperation(t *testing.T) {
	tests := []struct {
		code    []byte
		x, y, w int64
		q, r    int64
	}{
		{[]byte{0xA9, 0xB0, 0x02}, 9, 2, 4, 2, 6},
		{[]byte{0xA9, 0xB0, 0x02}, -9, 2, 4, -2, 2},
		{[]byte{0xA9, 0xB0, 0x02}, -9, 2, -4, -3, 2},
		{[]byte{0xA9, 0xB0, 0x02}, 0, 2, 4, 0, 4},
		{[]byte{0xA9, 0xB0, 0x02}, 9, 2, 0, 2, 2},
		{[]byte{0xA9, 0xB0, 0x02}, -5634879008887978, 2, 4, -1408719752221994, 0},
		{[]byte{0xA9, 0xB0, 0x02}, 11, 2, 4, 3, 2},
		{[]byte{0xA9, 0xB0, 0x02}, -11, 2, 4, -3, 6},
	}
	...
	for _, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatal("Failed MULADDRSHIFT#MOD execution:", err.Error())
			}
