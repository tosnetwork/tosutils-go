...
func init() {
	vm.List = append(vm.List, func() vm.OP { return MULRSHIFTRCODE(0) })
}
...
func MULRSHIFTRCODE(value int8) (op *helpers.AdvancedOP) {
	...
	op = &helpers.AdvancedOP{
		...
		Action: func(state *vm.State) error {
			...
			if x == nil || y == nil {
				return pushMaybeInt(state, legacyRShiftNaNResult(state.GlobalVersion, uint64(imm()), cppRoundNearest), false)
			}
		...
		BitPrefix:       helpers.BytesPrefix(0xA9, 0xB5),
		SerializeSuffix: serializeImmediate,
		NameSerializer: func() string {
			return fmt.Sprintf("%d MULRSHIFTR#", imm())
		},
