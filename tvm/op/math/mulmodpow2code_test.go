...
func TestMulmodpow2codeOperation(t *testing.T) {
	tests := []struct {
		code []byte
		x, y int64
		want int64
	}{
		{[]byte{0xA9, 0xB8, 0x03}, 9, 2, 2},
		{[]byte{0xA9, 0xB8, 0x03}, -9, 2, 14},
		{[]byte{0xA9, 0xB8, 0x03}, 0, 2, 0},
		{[]byte{0xA9, 0xB8, 0x03}, -5634879008887978, 2, 12},
		{[]byte{0xA9, 0xB8, 0x03}, 11, 2, 6},
		{[]byte{0xA9, 0xB8, 0x03}, -11, 2, 10},
		{[]byte{0xA9, 0xB8, 0x03}, 20, 2, 8},
		{[]byte{0xA9, 0xB8, 0x03}, -20, 2, 8},
	}
	...
	for _, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatal("Failed MULMODPOW2# execution:", err.Error())
			}
