...
func TestRshiftrcodemodOperation(t *testing.T) {
	tests := []struct {
		code []byte
		x    int64
		q, r int64
	}{
		{[]byte{0xA9, 0x3D, 0x02}, 10, 1, 2},
		{[]byte{0xA9, 0x3D, 0x03}, 10, 1, -6},
		{[]byte{0xA9, 0x3D, 0x02}, 5, 1, -3},
		{[]byte{0xA9, 0x3D, 0x02}, -7, -1, 1},
		{[]byte{0xA9, 0x3D, 0x03}, 0, 0, 0},
		{[]byte{0xA9, 0x3D, 0x21}, -5634879008887978, -327993, -2175620266},
		{[]byte{0xA9, 0x3D, 0x01}, -5634879008887978, -1408719752221994, -2},
		{[]byte{0xA9, 0x3D, 0x01}, -7, -2, 1},
		{[]byte{0xA9, 0x3D, 0x01}, -13, -3, -1},
	}
	...
	for _, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatal("Failed RSHIFTRCODEMOD execution:", err.Error())
			}
