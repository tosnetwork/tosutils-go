...
func init() {
	vm.List = append(vm.List, func() vm.OP { return LSHIFTDIVMODC() })
}
...
func LSHIFTDIVMODC() *helpers.SimpleOP {
	return &helpers.SimpleOP{
		Action: func(state *vm.State) error {
			...
			q := helpers.DivCeil(x.Mul(x, z.Lsh(bigIntOne, uint(z.Uint64()))), y)
			r := y.Sub(x, y.Mul(q, y))
		...
		Name:      "LSHIFTDIVMODC",
		BitPrefix: helpers.BytesPrefix(0xA9, 0xCE),
	}
}
