...
func TestMulrshiftccodemodOperation(t *testing.T) {
	tests := []struct {
		...
		{[]byte{0xA9, 0xBE, 0x03}, 9, 2, 2, -14},
		{[]byte{0xA9, 0xBE, 0x03}, -9, 2, -1, -2},
		{[]byte{0xA9, 0xBE, 0x03}, 0, 2, 0, 0},
		{[]byte{0xA9, 0xBE, 0x03}, -5634879008887978, 2, -704359876110997, -4},
		{[]byte{0xA9, 0xBE, 0x03}, 11, 2, 2, -10},
		{[]byte{0xA9, 0xBE, 0x03}, -11, 2, -1, -6},
	}
	...
	for _, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatal("Failed MULRSHIFTC#MOD execution:", err.Error())
			}
