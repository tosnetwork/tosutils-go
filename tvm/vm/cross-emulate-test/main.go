...
func RunGetMethod(params RunMethodParams, maxGas int64) (*RunResult, error) {
	...
	if err := tlb.Parse(&result, c); err != nil {
		return nil, err
	}
	return &result, nil
}
...
func prepareBlockchainConfig() *tvm.PreparedBlockchainConfig {
...
func main() {
	...
	if err = tlb.Parse(&resStack, res.Stack); err != nil {
		panic(4)
	}
	...
	goRes, err := vmx.Execute(MainContractCode, cell.BeginCell().EndCell(), tuple.Tuple{}, vm.NewGas(), s, tvm.ExecutionConfig{
		Config: prepareBlockchainConfig(),
	})
	if err != nil {
		panic(err.Error())
	}
